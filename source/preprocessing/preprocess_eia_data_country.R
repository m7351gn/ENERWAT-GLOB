...
eia.electricity$Country <- str_trim(eia.electricity$Country)

eia.electricity.nl <- eia.electricity %>% 
  filter(Country == 'Netherlands')
electricity.values <- t(eia.electricity.nl[3:44])
electricity.values <- as.numeric(electricity.values)

year.vector <- seq(1980,2021,1)

electricity.nl.df <- data.frame(
...
colnames(electricity.nl.df) <- c('Year', 'Total electricity (TWh)')

write.csv(electricity.nl.df, 'eia_electricity_net_consumption_TWh_NL.csv',
          row.names = F)
